...
This checklist complements the automated `npm run a11y` axe smoke suite. Run it before shipping ballot lookup, civic information, search, methodology, accessibility, privacy, and terms changes.

## Screen Reader Pass

- macOS VoiceOver: open `/`, `/search`, `/accessibility`, `/methodology`, `/privacy`; use `VO + Right Arrow` through each page, then jump by headings, landmarks, buttons, form controls, and links.
...
- Verify visible focus on skip links, search inputs, navigation links, ballot/candidate links, disclosure controls, form fields, and footer links.
