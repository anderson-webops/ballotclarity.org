...
import { getStateNameForAbbreviation } from "./official-election-tools.js";
...
function normalizeCountyName(value: string | null | undefined) {
	...
	return trimmed.replace(countySuffixPattern, "");
}

function normalizeCityName(value: string | null | undefined) {
	const trimmed = String(value ?? "").trim();
	return trimmed.replace(citySuffixPattern, "");
}
...
function resolveStateName(input: RepresentativeClassificationInput) {
	...
	const trimmedStateCode = String(input.stateCode ?? "").trim();
	if (trimmedStateCode) {
		const stateName = getStateNameForAbbreviation(trimmedStateCode) || trimmedStateCode;

		if (normalizeText(stateName) !== "united states")
			return stateName;
	}
