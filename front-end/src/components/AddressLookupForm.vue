...
<template>
	<form :class="[props.framed === false ? '' : 'surface-panel', compact ? 'p-5' : 'p-6 sm:p-7']" :aria-busy="isPending" @submit.prevent="handleSubmit">
		...
		<p v-if="compact" :id="privacyId" class="text-xs text-app-muted leading-6 mt-3 dark:text-app-muted-dark">
			Lookup data is used only to load civic results. Exact 5-digit ZIP lookups may be counted in a ZIP-only operations log; full street addresses are not added to that log.
		...
		<p v-else :id="privacyId" class="text-sm text-app-muted leading-6 mt-3 dark:text-app-muted-dark">
			Data use: your lookup is sent only to match ballot coverage. Exact 5-digit ZIP lookups may be counted in a ZIP-only operations log, but full street addresses and ZIP+4 entries are not added to that log. The raw lookup is not added to the public content archive or used for advertising, and the app saves only your selected location label and ballot-plan preferences locally in your browser. Read the
			<NuxtLink to="/privacy" class="underline underline-offset-3" prefetch-on="interaction">
				privacy notice
			</NuxtLink>.
		</p>
